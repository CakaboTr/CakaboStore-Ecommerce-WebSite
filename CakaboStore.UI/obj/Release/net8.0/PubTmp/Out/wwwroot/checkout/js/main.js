	...
	if (istenilenMiktar <= stock) {
		$.ajax({
			...
			success: function (data) {
				if (data.indexOf("~") == -1) {
					$('#modalSepet .modal-body').text(data + " ürününden " + istenilenMiktar + " adet sepete baþarýyla eklendi...")
		...
		alert("Maksimum " + stock + " adet ürün sipariþ edebilirsiniz")
}
...
function checkoutControl() {
	...
	if ($(".selectPaymentOption").val() == "") {
		mesaj = "Lütfen Bir Ödeme Seçeneði Belirtiniz";
	} if (mesaj != "") {
		alert(mesaj)
		return false;
	}
